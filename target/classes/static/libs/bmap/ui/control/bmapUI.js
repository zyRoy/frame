    ...
    $.widget("wy.wyDialog",{
        ...
        _createUI:function(){
            ...
            if(this.options.scroll){
                this.content = wyHelper.scroll(this.content);
            }
 ...
 *      }
    ...
    $.widget("wy.wyDatatable", {
        options: {
            ...
            trHoverInEvt: function() {},//默认鼠标进入行的事件处理
            trHoverOutEvt: function() {},//默认鼠标离开行的事件处理
            trClickEvt: function() {}//鼠标单击行的处理事件
        },
        //用于记录控件内全局对象
        container: null,
        header: null,
        ...
        _bindEvents: function () {
            ...
            //行的事件处理
            $(document).on("click",".wy-datatable-body tbody tr",this.options.trClickEvt);
            //排序事件
    ...
    $.widget("wy.wyGroupButton",{
        ...
        _init:function(){
            ...
            if(this.options.defaulted>-1){
                var defaultChild = this.element.children("a").eq(this.options.defaulted) ;
        ...
        _createButton:function(){
            ...
            if(this.options.button instanceof Array){
                ...
                for(var i=0;i<this.options.button.length;i++){
                    ...
                    this.buttons.push($("<span class='" + this.parts.button + " " + _buttonClass + "'>" + this.options.button[i] + "</span>").appendTo(this.container));
                }
            }else{
    ...
    $.widget("wy.wyPaginator", {
        ...
        _initEvent:function(){
            ...
            $firstpage.click(function(event){
                if(_self.element.find(".wy-paginator-firstpage>a").attr("disabled")=="disabled"){
                    return;
                }
            ...
            $previouspage.click(function(event){
                if(_self.element.find(".wy-paginator-previouspage>a").attr("disabled")=="disabled"){
                    return;
                }
            ...
            $nextpage.click(function(event){
                if(_self.element.find(".wy-paginator-nextpage>a").attr("disabled")=="disabled"){
                    return;
                }
                if($(".ui-scroll-container").length>0&&$(".ui-scroll-container > div").length==1){
                    return false
                }
                _self._setPage(_self._NowPage+1);
                _self._triggerPageChange(event);
            ...
            $lastpage.click(function(event){
                if(_self.element.find(".wy-paginator-lastpage>a").attr("disabled")=="disabled"){
                    return;
                }
    ...
    $.widget("wy.wySidepanel",{
        ...
        show:function(){
            ...
            this.element.animate(animateData, this.options.anitime);
            this.isShowing = true;
        ...
        hide:function(){
            ...
            var endtime = 200;
            if (this.options.anitime >= 200) {
                endtime = this.options.anitime - 200;
            }
            this.element.animate(animateParam, endtime);
    ...
    $.widget("wy.wyTabview",{
        ...
        _createNavButton:function(){
            return $("<div style='right: inherit;left: 10px;' class='"+ this.parts.navButton +"'><i class=' "+ this.parts.previousTab + " "  + icoDef.previous +"'></i></div>" +
            "<div class='"+ this.parts.navButton +"'><i class='"+ this.parts.nextTab + " " + icoDef.next +"'></i></div>");
        },
        //创建页签导般下拉
        ...
        _appendNewTab:function(tab){
            //页签
            var $tab = $("<li data-id="+tab.key+" class='"+ this.parts.tab +"'>" + tab.title + "</li>");
            if(tab.data){
        ...
        add:function(tab,ignoreSame){
            ...
            //判断当前标题的页签是否存在
            this.header.find("." + this.parts.tab).each(function(){
                if($.trim(tab.title) === $.trim($(this).children("div").text())&&tab.key&&$.trim(tab.key) === $.trim($(this).data("id"))){
                    if(tab.reload){
    ...
    $.widget("wy.wyTabview2",{
        ...
        _appendNewTab:function(tab){
            //页签
            var $tab = $("<li data-id = '"+tab.data.id+"' class='"+ this.parts.tab +"'>" + tab.title + "</li>");
            if(tab.data){
                $tab.data("data",tab.data);
            }
            //是否能关闭
        ...
        add:function(tab,ignoreSame){
            ...
            //判断当前标题的页签是否存在
            this.header.find("." + this.parts.tab).each(function(){
                console.log($.trim($(this).data("id")))
                if(tab.data.id){
                    if($.trim(tab.title) === $.trim($(this).children("div").text())&&tab.data.id==$.trim($(this).data("id"))){
                        if(tab.reload){
                            var index = _self.header.find("." + _self.parts.tab).index($(this));
                            _self.content.children("div:eq("+ index +")").remove();
                            $(this).remove();
    ...
    $.widget("wy.wyTree",{
        options:{
            ...
            itemRenderer:function(data,$node){
                return data[this.text] ; //$node = li>span
            ...
            itemCheck: function() {}, //checkbox被选中时触发的事件
            mutilpleSelect: false, //是否支持多选，如果有子节点清除自节点的选中状态
            nodeSelected: function() {}//节点选中时触发事件
        },
        _content:null,//节点容器
        _dsTree:null,
        _checkBind:true,
        ...
        _init:function(){
            ...
            this._initDrag();
            //默认根节点被选中
            this.options.hideNode && this.hideNode(this.options.hideNode);
        },
        _initDrag:function(){
            //=======================================================
            //drag
        ...
        _bindNodeEvent:function($node){
            ...
            $node.children("span").click(function(event){
                ...
                if (_self.options.mutilpleSelect) {
                    var hasCurrent = $(this).hasClass("current");
                    var parentLis = $(this).parents("li");
                    parentLis.each(function(index, item){
                        $(item).find("> .wy-tree-span").removeClass("current");
                    });
                    $(this).closest("li.wy-tree-node").find(".wy-tree-span").removeClass("current");
                    hasCurrent ? $(this).removeClass("current") : $(this).addClass("current");
                    if($(".wy-tree-span.current").length <= 0) {
                ...
                var selectedNodes = [];
                $(".wy-tree-span.current").each(function(index, item) {
                    var tempObj = $(item).closest("li").data("data");
                    delete tempObj.children;
                    selectedNodes.push(tempObj);
                });
                _self.options.nodeSelected(selectedNodes);
        ...
        _bindCheckEvent:function($check){
            ...
            $check.wyCheckbox({
                change:function(e,d){
                    ...
                    _self.options.itemCheck(_self.getCheckData());
                }
            });
        },
        ...
        _nodeFilter:function(data,key){
            ...
            if(data[this.options.text] && data[this.options.text].indexOf(key)>-1){
                return true;
            }
            return false;
        },
        ...
        hideNode: function(d2) {
            d2 = d2 || this.options.hideNode;
            if (d2) {
                var level = $("ul[level="+ d2 +"]");
                level.find(".wy-tree-toggler").trigger("click");
            }
        },
    ...
    $.widget("wy.bmitree", {
        ...
        options: {
            ...
            selectionMode: null,
            icons: null,
            check:true, //是否有checkbox
            checkBind:true, //父子节点check时是否联动
            isSimpleData:false,//是否是线性的数据结构
            checkChange:null,//当有checkbox选中时触发
            expandAll:false//是否展开全部节点
        },
        _craeteTree: function() {
            if(this.options.isSimpleData){
                this._simpleDataToTree();
            }
            this._create();
        },
        ...
        _renderNode: function(node, container) {
            var _self = this;//使用局部变量转换this
            //修复当设置树全部展开时，展开不正确bug
            if(this.options.expandAll==true){
                node.expanded = true ;
            }
            ...
            /**修复初始为展开节点状态不正确的bug*/
            if(node.expanded){
                nodeElement.addClass("pui-treenode-expanded");
            }
            ...
            if(this.options.check){
                contentElement.append("<input type='checkbox'/>");
                contentElement.find("input[type=checkbox]").bmcheckbox({
                    change:function(event,checked){
                        _self._checkChange(event,checked);
                    }
                });

                contentElement.find("input[type=checkbox]").each(function(){
                    $(this).closest(".pui-chkbox").click(function(){
                        event.stopPropagation();
                    });
                });
            }
            contentElement.append('<span class="pui-treenode-icon"></span>')
                .append('<span class="pui-treenode-label ui-corner-all">' + node[_self.options.label || "label"] + '</span>')
            ...
            if(!leaf) {
                ...
                if(!node.expanded && this.options.expandAll==false) {
        ...
        _checkChild:function(node,checked){//$node=pui-treenode
            var $node = node instanceof jQuery ? node : $(node) ;
            if(checked==true){
                $node.find(".pui-treenode-content").find("input[type=checkbox]").bmcheckbox("check");
            }else{
                $node.find(".pui-treenode-content").find("input[type=checkbox]").bmcheckbox("uncheck");
            }
        },
        _checkParent:function(node,checked){
            var $node = node instanceof jQuery ? node : $(node) ;
            var $ul = $node.parent("ul");
            if($ul && $ul.hasClass("pui-treenode-children")){
                $node = $ul.parent("li");
                var childNum = $ul.find("input[type=che_checkChangeckbox]").length;
                var checkChildNum = $ul.find(".ui-icon-check").length;

                if(checkChildNum==0){
                    $node.find(".pui-chkbox-box:first").removeClass("ui-state-active");
                    $node.find("input[type=checkbox]").bmcheckbox("uncheck");
                }else if(childNum!=checkChildNum){
                    $node.find("input[type=checkbox]:first").bmcheckbox("uncheck");
                    $node.find(".pui-chkbox-box:first").addClass("ui-state-active");
                }else{
                    $node.find("input[type=checkbox]").bmcheckbox("check");
                }
                if($node){
                    this._checkParent($node,checked);
                }
            }
        },
        _checkChange:function(event,checked){
            if(!this.options.checkBind){
                return;
            }
            var $nodes = $(event.target).parents(".pui-treenode");
            if($nodes){
                this._checkChild($nodes.get(0),checked);
                this._checkParent($nodes.get(0),checked);
            }
            var data = this.getCheckedDatas(false) ;
            this.options.checkChange(data);
        },
        ...
        getCheckedDatas:function(onlyLeaf){//获取被选中的节点列表,onlyLeaf为true时只返回叶节点
            var $nodes , datas = [] , $item ;
            $nodes = onlyLeaf==true ? this.rootContainer.find("li .pui-treenode .pui-treenode-leaf") : this.rootContainer.find("li .pui-treenode");
            for(var i=0;i<$nodes.length;i++){
                $item = $nodes.get(i) instanceof jQuery ? $nodes.get(i) : $($nodes.get(i)) ;
                if($item.find("input[type=checkbox]:first").bmcheckbox("isChecked")){
                    datas.push($item.data('puidata'));
                }
            }
            return datas;
        },
        //线性数据结构转树状结构
        _simpleDataToTree:function(){
            if(!this.options.nodes){
                return ;
            }
            var pids = this._iDs2Array(this.options.nodes);
            this.options.nodes = this._toTree(this.options.nodes, pids);
            console.log(JSON.stringify(this.options.nodes));

            if(this.options.hehe){

                var heheChildren = null;

                if(this.options.nodes && this.options.nodes.length>0){
                    for(var i=0;i<this.options.nodes.length;i++){
                        this.options.nodes[i].expanded = true ;

                        if(this.options.nodes[i].children){
                            heheChildren = this.options.nodes[i].children ;
                            for(var j=0;j<heheChildren.length;j++){
                                heheChildren[j].expanded = true ;
                            }
                        }
                    }
                }
        ...
        refresh: function(data) {
            ...
            var nodes = data ;
            var pids = this._iDs2Array(data);
            if(this.options.isSimpleData){
                nodes = this._toTree(data, pids);
            }
            this._renderNodes(nodes, this.rootContainer);
        },
        ...
        _toTree : function(data, pids, pid) {
            var _self = this, tree = [], root;
            var id = _self.options.id, npid = _self.options.pid;
            if (!data) return tree;
            //第一次,找根节点ID
            if (!pid) {
                for (var i = 0; i < data.length; i ++) {
                    if (!data[i][npid] || data[i][npid] == '-1') {
                        tree.push(data[i]);
                    }
                }
            } else {
            ...
            for (var i = 0; i < tree.length; i ++) {
                if (pids.indexOf(tree[i][id]) > 0) {
                    tree[i].children = this._toTree(data, pids, tree[i][id]);
                } else {
                    break;
                }
            }
        ...
        _iDs2Array: function(data) {
            var tempData = [];
            for (var i = 0; i < data.length; i ++) {
                if (tempData.indexOf(data[i][this.options.pid]) < 0) {
                    tempData.push(data[i][this.options.pid]);
                }
            }
            return tempData;
        },
        _formatData: function(data) {
            ...
            var tmpData = {};
            for (var i = 0; i < data.length; i ++) {
                tmpData[data[i][_self.options.id]] = data[i] || {};
            }
            return tmpData;
        },
        _toTrees:function(data,pid,idname,pidname) {
            var tree = [],temp,temp2,mark,pids=[], _self = this;
            idname = idname ? idname : _self.options.id || "ID";
            pidname = pidname ? pidname : _self.options.pid || "PID";
            ...
            if(pid==undefined || pid==null){
                ...
                for(var i=0;i<data.length;i++){
                    ...
                    for(var j=0;j<data.length;j++){
                        temp2 = data[j];
                        if(temp[idname]!=temp2[idname] && temp[pidname]==temp2[idname] || temp[pidname] == '-1'){
                            mark = true ;
                            break;
                        }
                    }
            ...
            if(pid){
                for(var i=0;i<data.length;i++){
                    temp = data[i];
                    if(temp[pidname]==pid){
                        //console.info(pid + ":" + temp["NAME"]);
                        temp.children = this._toTree(data,temp[idname]);
                        tree.push(temp);
                    }
                }
            }
    ...
    $.widget("bmui.bmtree", $.primeui.puitree,{
        options: {
            hehe:false,//
            nodes: null,
            lazy: false,
        ...
        _checkParent:function(node,checked){
            ...
            if($ul && $ul.hasClass("pui-treenode-children")){
                $node = $ul.parent("li");
                var childNum = $ul.find("input[type=checkbox]").length;
                var checkChildNum = $ul.find(".ui-icon-check").length;
        ...
        _simpleDataToTree:function(){
            ...
            this.options.nodes = this._toTree(this.options.nodes);
        ...
        _toTree:function(data,pid,idname,pidname) {
            var tree = [],temp,temp2,mark,pids=[];
            idname = idname ? idname : "ID";
            pidname = pidname ? pidname : "PID";
